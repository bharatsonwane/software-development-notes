...
Conditions use **truthy/falsy** coercion (see File 03, section 4). A block runs when its condition evaluates to a truthy value.
...
### 3.3. Labeled break & continue (brief)
...
**4.1. What is the difference between `if` and ternary?**  
...
**4.6. What is the difference between `for...in` and `for...of`?**  
...
**4.7. What do `break` and `continue` do?**  
A: `break` exits the innermost loop or `switch`. `continue` skips the rest of the **current** loop iteration and starts the next one.

**4.8. Why does `for (let i = 0; ...)` behave differently from `for (var i = 0; ...)` in closures?**  
A: `let` creates a **new binding per iteration**. `var` is function-scoped, so all callbacks share one `i`. See File 02, section 2.3.
